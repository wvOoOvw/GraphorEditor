...
deleteDeepDirectory(path.join(__dirname, './build'))
  ...
  const webpackConfig_ = (name) => {
    return Object.assign({}, webpackConfig, {
      output: {
        libraryTarget: 'umd',
        filename: `${name}.js`,
        path: path.join(__dirname, './build/element')
      },
    })
  }

  const dir = fs.readdirSync(path.join(__dirname, `../element/core`))

  const output = (name) => `
    import Render from '../element/core/${name}/Render.jsx'; 
    import license from '../element/core/${name}/license.js'; 
...
const elements = async () => {
  ...
  const output = `
    ...
    ${dir.map(i => { const name = i.replace('-', ''); return `import ${name}Render from '../element/core/${i}/Render.jsx'; import ${name}license from '../element/core/${i}/license.js'; list.push({ Render: ${name}Render, license: ${name}license}); ` }).join('')}
...
const render = async () => {
  ...
  const output = `
    import GraphPure from '../main/view/Page.GraphPure'
...
const html = async () => {
  ...
  fs.mkdirSync(path.join(__dirname, './build/html'))

  fs.writeFileSync(path.join(__dirname, './build/html/index.html'), h)
}
